...
namespace Dumpper.Writer
{
    internal sealed partial class Printer
    {
        ...
        private static IRenderable GenerateColumn<T>(T instance, PropertyInfo[] propertiesInfo, string className)
        {
            var tree = Printer.GenerateTree(instance, propertiesInfo, className);
            return tree;
        }
        ...
        private static Table? CreateTable<TKey, TValue>(Dictionary<TKey, TValue> instance)
        {
            ...
            foreach (var item in instance)
            {
                ...
                else
                {
                    dictionaryTable.AddRow(
                        new Markup(item.Key?.ToString() ?? string.Empty),
                        new Markup(item.Value?.ToString() ?? string.Empty));
                }
            }
